...
(function ($, elementor) {
    ...
    var widgetRemotePagination = function ($scope, $) {
        ...
        if (!$settings.remoteId) {
            // return;
            // try to auto detect
            var $parentSection = $scope.closest('.elementor-section, .e-container--column');

            $settings['remoteId'] = $parentSection;
        }

        if ($($settings.remoteId).find('.swiper-container, .swiper-container-initialized').length <= 0) {
            if (editMode == true) {
        ...
        $(document).ready(function () {
            setTimeout(() => {
                const swiperInstance = $($settings.remoteId).find('.swiper-container, .swiper-container-initialized')[0].swiper;
    ...
    jQuery(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/prime-slider-remote-pagination.default', widgetRemotePagination);
    });
